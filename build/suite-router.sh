...
if echo "$CHANGED" | grep -qE "^src/components/mcp/"; then
  SPECS="$SPECS mcp-setup-wizard.spec.ts mcp-overview.spec.ts mcp-wizard.spec.ts mcp-resource-pages.spec.ts mcp-external-wizard.spec.ts mcp-inspector.spec.ts"
fi
