...
`);
  ...
  test('Gateway Attached tab shows HTTPRoute and AuthPolicy', { tag: '@nightly' }, async ({
    page,
  }) => {
    await gotoPage(
      page,
      `/k8s/ns/${namespace}/gateway.networking.k8s.io~v1~Gateway/${gateway}`,
    );
  ...
  test('navigates from Attached tab to HTTPRoute details', { tag: '@nightly' }, async ({
    page,
  }) => {
    await gotoPage(
      page,
  ...
  test('navigates from Attached tab to AuthPolicy details', { tag: '@nightly' }, async ({
    page,
  }) => {
    await gotoPage(
      page,
