...
// Helper to create an HTTPRoute for testing (assumes httproutes can be created)
async function createTestHTTPRoute(page: Page, namespace: string, name: string): Promise<void> {
  // Navigate to HTTPRoute create page and create a simple HTTPRoute
  // This is a placeholder - actual implementation depends on your HTTPRoute creation flow
  // For now, we assume HTTPRoutes exist or skip this step
}

// Helper to delete an APIProduct via UI
async function deleteAPIProductViaUI(
  page: Page,
  namespace: string,
  apiProductName: string,
): Promise<void> {
  await navigateToAPIProducts(page, namespace);
  await page.waitForLoadState('networkidle');

  // Find the row with the APIProduct
  const row = page.locator(`tr:has-text("${apiProductName}")`);
  ...
  await confirmInput.fill(apiProductName);
...
test.describe('APIProduct CRUD Operations', () => {
  ...
  test('should navigate from list page to create page via Create button', async ({ page }) => {
    ...
    // Wait for page to load and verify we're on the list page
    await expect(page.locator('h1:has-text("API Products")')).toBeVisible({ timeout: 15000 });
  ...
  test('should validate resource name format', async ({ page }) => {
    ...
    for (const validName of validNames) {
      ...
      // Save button might still be disabled due to missing required fields (HTTPRoute)
      // but it shouldn't be disabled due to invalid resource name format
      // We can't fully test this without selecting an HTTPRoute
    }
  });

  test('should sync form to YAML correctly', async ({ page }) => {
    await navigateToAPIProductCreate(page, TEST_NAMESPACE);
    await page.waitForLoadState('networkidle');
    ...
    // Get YAML content (this depends on how ResourceYAMLEditor works)
    // Monaco editor - use innerText to preserve line breaks
    ...
    // Note: version and documentation URLs may not sync immediately due to React state timing
  ...
  test('should disable Deprecated and Retired statuses', async ({ page }) => {
    ...
    // Get all options
    const options = await publishStatusSelect.locator('option').all();

    // Find Draft and Published options - should be enabled
    const draftOption = await publishStatusSelect.locator('option[value="Draft"]');
    expect(await draftOption.isDisabled()).toBe(false);

    const publishedOption = await publishStatusSelect.locator('option[value="Published"]');
    expect(await publishedOption.isDisabled()).toBe(false);

    // Find Deprecated and Retired options - should be disabled
    const deprecatedOption = await publishStatusSelect.locator('option[value="Deprecated"]');
    expect(await deprecatedOption.isDisabled()).toBe(true);

    const retiredOption = await publishStatusSelect.locator('option[value="Retired"]');
  ...
  test('should delete APIProduct with confirmation', async ({ page }) => {
    ...
    await expect(page.locator('text=Delete API Product')).toBeVisible({ timeout: 10000 });
