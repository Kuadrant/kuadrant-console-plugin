...
export const removeUndefinedFields = (obj: any) => {
  return Object.keys(obj).reduce((acc, key) => {
    ...
    if (value !== undefined) {
      acc[key] =
        typeof value === 'object' && !Array.isArray(value) ? removeUndefinedFields(value) : value;
    }
    return acc;
  }, {});
};

export const convertMatchLabelsArrayToObject = (
  matchLabelsArray: MatchLabel[],
): { [key: string]: string } => {
...
export const convertMatchLabelsObjectToArray = (matchLabelsObject: {
  [key: string]: string;
}): MatchLabel[] => {
