...
import { GatewayResource } from '../components/gateway/types';
...
const ParentReferencesSelect: React.FC<ParentReferencesSelectProps> = ({
  ...
  const [availableGateways, setAvailableGateways] = React.useState<GatewayResource[]>([]);
  ...
  const validateGateway = (gateway: GatewayResource): string | null => {
  ...
  const validateListener = (gateway: GatewayResource, listenerName: string): string | null => {
  ...
  return (
    <FormGroup
      label={
        <span>
          {t('Parent references')} <span style={{ color: 'var(--pf-v6-global--danger-color--100)' }}>*</span>
        </span>
      }
