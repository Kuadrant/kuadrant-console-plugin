...
const listPrompts = async (
  ...
  try {
    ...
    if (err instanceof MCPUnauthorizedError || err instanceof MCPSessionExpiredError) {
      throw err;
    }
...
const MCPInspectorPage: React.FC = () => {
  ...
  return <MCPInspectorSession key={activeNamespace} activeNamespace={activeNamespace} />;
};

const MCPInspectorSession: React.FC<{ activeNamespace: string }> = ({ activeNamespace }) => {
  const { t } = useTranslation('plugin__kuadrant-console-plugin');
  const resolvedNamespace = activeNamespace === ALL_NS ? undefined : activeNamespace;

  const [extensions, extensionsLoaded, extensionsError] = useK8sWatchResource<
    MCPGatewayExtension[]
  >({
  ...
  const [registrations, , registrationsError] = useK8sWatchResource<MCPServerRegistration[]>({
  ...
  const [protocolMode, setProtocolMode] = React.useState<MCPProtocolMode>('auto');
  const [connection, setConnection] = React.useState<MCPConnection | null>(null);
  ...
  const pendingOperations = React.useRef(new Set<(busy: boolean) => void>());

  React.useEffect(
    () => () => {
      // Namespace changes unmount this session, including its credentials.
  ...
  const openSession = async (inspectorEndpoint: string, mode: MCPProtocolMode, bearer?: string) => {
    const client = new MCPClient(inspectorEndpoint, {
    ...
    return { client, negotiated, tools: listed.tools ?? [], promptState };
  };

  const handleConnect = async (inspectorEndpoint: string, bearer?: string, mode = protocolMode) => {
    ...
    clientRef.current = null;
    try {
      const session = await openSession(inspectorEndpoint, mode, bearer);
      if (!isCurrent()) {
        return;
      }
      clientRef.current = session.client;
      setSessionId(session.negotiated.sessionId);
      setConnection(session.negotiated);
      setConnected(true);
  ...
  const resetConnection = ({ preserveActiveSection = false } = {}) => {
    invalidateSession();
  ...
  const handleProtocolChange = (_event: React.FormEvent<HTMLSelectElement>, value: string) => {
    const mode = value as MCPProtocolMode;
    setProtocolMode(mode);
    const token = bearerToken.trim();
    resetConnection({ preserveActiveSection: true });
    // The gateway is unchanged, so its in-memory credential can be reused.
    setBearerToken(token);
    if (selected && isReady(selected)) {
      void handleConnect(proxyEndpoint(selected), token, mode);
    }
  };
  ...
  const runOnSession = async <T,>(
    ...
    if (!client || pendingOperations.current.has(setBusy)) {
      return;
    }
    pendingOperations.current.add(setBusy);
    const attempt = connectAttempt.current;
    const isCurrent = () => attempt === connectAttempt.current;
    setError('');
    setSessionExpired(false);
    ...
    try {
      ...
      if (err instanceof MCPSessionExpiredError) {
        ...
        setBearerToken('');
        setAuthRejected(false);
        if (selected) setAuthChallenge(proxyEndpoint(selected));
      } else {
        setError(err instanceof Error ? err.message : String(err));
      }
    } finally {
      if (isCurrent()) {
        pendingOperations.current.delete(setBusy);
        setBusy(false);
      }
    }
  };
    ...
    runOnSession(
      ...
      (exchange) => {
        ...
        setStats((current) => ({
          ...
          warnings:
            current.warnings +
            (exchange.result.isError || exchange.result.resultType === 'input_required' ? 1 : 0),
        }));
      },
      () => setCallExchange(null),
    );
    ...
    runOnSession(
      ...
      (exchange) => {
        ...
        setStats((current) => ({
          ...current,
          requests: current.requests + 1,
          warnings: current.warnings + (exchange.result.resultType === 'input_required' ? 1 : 0),
        }));
      },
      () => setPromptExchange(null),
    );
    ...
    <>
      ...
      <PageSection className="kuadrant-mcp-inspector-page">
        <Stack hasGutter>
                ...
                <Grid>
                  <GridItem lg={6} className="kuadrant-mcp-inspector-page__connection-segment">
                    <Form className="kuadrant-mcp-inspector-page__connection-form">
                      ...
                      <FormGroup label={t('Protocol')} fieldId="mcp-inspector-protocol">
                        <FormSelect
                          id="mcp-inspector-protocol"
                          value={protocolMode}
                          onChange={handleProtocolChange}
                          aria-label={t('MCP protocol')}
                        >
                          <FormSelectOption value="auto" label={t('Auto (prefer 2026)')} />
                          <FormSelectOption
                            value="2026-07-28"
                            label={t('2026-07-28 (stateless)')}
                          />
                          <FormSelectOption value="2025-11-25" label={t('2025-11-25 (legacy)')} />
                        </FormSelect>
                  ...
                  >
                    ...
                    {connection && (
                      ...
                      >
                        {t('Protocol')}: {connection.protocolVersion} ·{' '}
                        {connection.protocolVersion === '2026-07-28'
                          ? t('Stateless')
                          : connection.sessionId
                          ? t('Session-based')
                          : t('Sessionless legacy')}
                      </Content>
                    )}
                    {sessionId && connected && (
                      <Content
          ...
          {extensionsError && (
            <StackItem>
              <Alert variant="danger" isInline title={t('Unable to load MCP gateway extensions')}>
                {extensionsError.message || String(extensionsError)}
              </Alert>
            </StackItem>
          )}
          {registrationsError && (
            <StackItem>
              <Alert variant="warning" isInline title={t('MCP server names are unavailable')}>
                {t(
                  'Server names require access to MCP server registrations across namespaces. You can still connect and run tools.',
                )}
              </Alert>
            </StackItem>
          )}
