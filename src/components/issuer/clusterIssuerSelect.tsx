...
const ClusterIssuerSelect: React.FC<ClusterIssuerSelectProps> = ({ selectedClusterIssuer, onChange }) => {
  ...
  React.useEffect(() => {
    if (clusterIssuerLoaded && !clusterIssuerError && Array.isArray(clusterIssuerData)) {
      setClusterIssuers(clusterIssuerData.map((clusterIssuer) => ({
  ...
  return (
    <>
      <FormGroup label={t('ClusterIssuer API Target Reference')} isRequired fieldId="clusterissuer-select">
        <FormSelect
        ...
        >
          <FormSelectOption key="placeholder" value="" label={t('Select an ClusterIssuer')} isPlaceholder />
