...
func loadConfig() (config, error) {
	...
	return config{
		...
		upstreamCAFile:        os.Getenv("MCP_PROXY_CA_FILE"),
		allowedOrigins:        os.Getenv("MCP_PROXY_ALLOWED_ORIGINS"),
...
func newServer(cfg config, logger *slog.Logger) (*server, error) {
	...
	for _, value := range strings.Split(cfg.allowedOrigins, ",") {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid MCP_PROXY_ALLOWED_ORIGINS entry: %w", err)
		}
	...
	if !cfg.kubernetesSkipVerify {
		roots, err := loadCertPool(cfg.kubernetesCAFile)
		if err != nil {
			return nil, fmt.Errorf("load Kubernetes CA: %w", err)
		}
	...
	if cfg.upstreamCAFile != "" {
		roots, err := loadCertPool(cfg.upstreamCAFile)
		if err != nil {
			return nil, fmt.Errorf("load MCP gateway CA: %w", err)
		}
		upstreamTransport.TLSClientConfig.RootCAs = roots
	}
...
func (s *server) proxyMCP(writer http.ResponseWriter, request *http.Request) {
	...
	if !validNamespace(namespace) || !validResourceName(name) {
		writeJSONError(writer, http.StatusBadRequest, "invalid MCPGatewayExtension namespace or name")
		return
	}
	...
	origin, err := parseOrigin((&url.URL{Scheme: target.Scheme, Host: target.Host}).String())
	if err != nil || (len(s.allowedOrigins) > 0 && !s.allowedOrigins[origin]) {
		writeJSONError(writer, http.StatusForbidden, "MCP gateway origin is not permitted by MCP_PROXY_ALLOWED_ORIGINS; contact your administrator")
		return
	}
	...
	copyRequestHeader(request.Header, upstreamRequest.Header, "Mcp-Method")
	copyRequestHeader(request.Header, upstreamRequest.Header, "Mcp-Name")
	for header := range request.Header {
		if strings.HasPrefix(strings.ToLower(header), "mcp-param-") {
			copyRequestHeader(request.Header, upstreamRequest.Header, header)
		}
	}
	if mcpAuthorization != "" {
...
func (s *server) resolveMCPEndpoint(ctx context.Context, namespace, name, authorization string) (string, int, error) {
	...
	baseURL.Path = path.Join(apiBasePath, "apis/mcp.kuadrant.io/v1/namespaces", namespace, "mcpgatewayextensions", name)
	...
	if !validNamespace(targetNamespace) || !validResourceName(extension.Spec.TargetRef.Name) {
		return "", http.StatusBadGateway, errors.New("MCPGatewayExtension has an invalid Gateway reference")
	}
...
// The optional allowlist restricts destinations beyond the admin-managed Gateway
// and extension configuration. Origins must be exact; wildcards and URL paths are rejected.
func parseOrigin(value string) (string, error) {
	...
	if err != nil || u.Opaque != "" || u.User != nil || u.RawQuery != "" || u.ForceQuery || u.Fragment != "" ||
		...
		return "", errors.New("expected an exact http(s) origin without credentials, query or path")
	}
	port := u.Port()
	if port == "" {
		port = "80"
	...
	return u.Scheme + "://" + net.JoinHostPort(strings.ToLower(u.Hostname()), strconv.Itoa(number)), nil
}

func loadCertPool(filename string) (*x509.CertPool, error) {
	caPEM, err := os.ReadFile(filename)
	if err != nil {
		return nil, err
	}
...
func allowedMCPMethod(method string) bool {
	switch method {
	case "server/discover", "initialize", "notifications/initialized", "tools/list", "tools/call", "prompts/list", "prompts/get":
		return true
	default:
		return false
	}
